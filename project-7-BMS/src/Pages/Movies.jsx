...
import { FaFilm, FaStar, FaLanguage, FaSearch, FaTrashAlt } from 'react-icons/fa';
...
const Movies = () => {
  ...
  useEffect(() => {
    ...
    // Apply filters
    if (filters.genre) result = result.filter(movie => movie.genre.includes(filters.genre));
    if (filters.language) result = result.filter(movie => movie.language === filters.language);
    if (filters.rating) result = result.filter(movie => movie.rating >= parseFloat(filters.rating));

    setFilteredMovies(result);
    setCurrentPage(1);
  }, [movies, searchParam, filters]);
  ...
  const clearFilters = () => setFilters({ genre: '', language: '', rating: '' });
  ...
  return (
    <Container className="my-5 ">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap">
        <h2 className="mb-2 mt-5"><FaFilm className="me-2 text-danger" />Now Showing</h2>
        <Link to="/add-movie" className="btn btn-primary mb-2 mt-5">
          <MdMovieCreation className="me-1" /> Add Movie
        </Link>
      </div>
      ...
      {filteredMovies.length === 0 ? (
        <div className="text-center py-5">
          <h4>No movies found <FaSearch /></h4>
          <Link to="/add-movie" className="btn btn-primary mt-3">Add Your First Movie</Link>
        </div>
      ) : (
        <>
          <Row className="g-4">
